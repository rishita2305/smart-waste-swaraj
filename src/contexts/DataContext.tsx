...
export interface DataContextType {
  ...
  updateWasteListing: (
    listingId: string,
    updatedFields: Partial<WasteListing>
  ) => Promise<void>;
}
...
export const DataProvider = ({ children }: { children: ReactNode }) => {
    ...
    const fetchInitialData = async () => {
      ...
      const dummyUsers: User[] = [
        ...
        }, // Gurgaon
        {
          ...
          name: "Diana Buyer",
          userType: "generator",
          location: { latitude: 28.6, longitude: 77.05 },
        }, // Another generator/buyer
      ];
      setUsers(dummyUsers);
      ...
      const dummyListings: WasteListing[] = [
        {
          ...
          quantity: "2",
          unit: "kg",
          description: "Fresh kitchen waste, suitable for composting",
          location: {
            latitude: 27.1751,
            longitude: 78.0421,
          ...
          status: "pending",
          createdAt: (Date.now() - 86400000 * 2).toString(),
          itemType: "waste",
          wasteCategory: "biodegradable",
          imageUrl:
            "https://placehold.co/400x300/a8e6cf/000000?text=Biodegradable+Waste",
        },
        {
          id: "wl2",
          userId: "gen1",
          wasteType: "Plastic Bottles (PET)",
          quantity: "500",
          unit: "grams",
          description: "Clean, crushed plastic bottles",
          location: {
            latitude: 27.18,
            longitude: 77.99,
          ...
          status: "assigned",
          assignedCollectorId: "col1",
          createdAt: (Date.now() - 86400000 * 3).toString(),
          itemType: "waste",
          wasteCategory: "non_biodegradable",
          imageUrl:
            "https://placehold.co/400x300/87ceeb/000000?text=Plastic+Bottles",
          comments: [
            {
              id: "c1",
              userId: "col1",
              userName: "Bob Collector",
              text: "On my way to pick this up!",
              createdAt: (Date.now() - 86400000 * 0.5).toString(),
            },
          ],
        },
        {
          ...
          quantity: "10",
          unit: "kg",
          description: "Bundle of old newspapers and magazines",
          location: {
            latitude: 27.2,
            longitude: 77.98,
          ...
          status: "pending",
          createdAt: (Date.now() - 86400000 * 1).toString(),
          itemType: "old_item",
          wasteCategory: "recyclable_paper",
          imageUrl:
            "https://placehold.co/400x300/ffd700/000000?text=Old+Newspapers",
          price: 150,
          comments: [
            {
              id: "c2",
              userId: "buyer1",
              userName: "Diana Buyer",
              text: "Interested! Can I pick up tomorrow?",
              createdAt: (Date.now() - 86400000 * 0.2).toString(),
            },
          ],
        },
        {
          id: "wl4",
          userId: "gen1",
          wasteType: "Used Batteries",
          quantity: "1",
          unit: "bag",
          description: "Assorted used AA/AAA batteries",
          location: {
            latitude: 27.195,
            longitude: 78.01,
            address: "Agra Cantt, Agra",
          },
          status: "completed",
          assignedCollectorId: "col1",
          createdAt: (Date.now() - 86400000 * 5).toString(),
          completedAt: (Date.now() - 86400000 * 4).toString(),
          itemType: "waste",
          wasteCategory: "hazardous",
          imageUrl:
            "https://placehold.co/400x300/ff6347/000000?text=Used+Batteries",
        },
        {
          id: "wl5",
          userId: "gen2",
          ...
          quantity: "1",
          unit: "unit",
          description: "3-seater sofa, good condition, needs new cover.",
          location: {
            latitude: 27.16,
            longitude: 78.03,
            address: "Taj East Gate, Agra",
          },
          status: "pending",
          createdAt: (Date.now() - 86400000 * 0.8).toString(),
          itemType: "old_item",
          imageUrl: "https://placehold.co/400x300/8b4513/ffffff?text=Old+Sofa",
          price: 1500,
        },
        {
          id: "wl6",
          userId: "gen1",
          wasteType: "Mixed Food Waste",
          quantity: "3",
          unit: "kg",
          description: "Daily kitchen scraps and leftover food",
          location: {
            latitude: 27.17,
            longitude: 78.0,
            address: "Agra Fort Area, Agra",
          },
          status: "pending",
          createdAt: (Date.now() - 86400000 * 0.1).toString(),
          itemType: "waste",
          wasteCategory: "biodegradable",
          imageUrl:
            "https://placehold.co/400x300/98fb98/000000?text=Food+Waste",
        },
      ];
  ...
  const assignCollectorToListing = async (
    listingId: string,
    collectorId: string
  ) => {
    setWasteListings((prev) =>
      prev.map((l) =>
        l.id === listingId
          ? { ...l, status: "assigned", assignedCollectorId: collectorId }
          : l
      )
    );
  ...
  const completeListing = async (listingId: string) => {
    setWasteListings((prev) =>
      prev.map((l) =>
        l.id === listingId
          ? { ...l, status: "completed", completedAt: Date.now().toString() }
          : l
      )
    );
    await new Promise((resolve) => setTimeout(resolve, 300));
  ...
  const createWasteListing = async (
    newListingPartial: Omit<
      WasteListing,
      | "id"
      | "createdAt"
      | "status"
  ...
  const addCommentToListing = async (
    listingId: string,
    newCommentPartial: Omit<Comment, "id" | "createdAt">
  ) => {
  ...
  const updateWasteListing = async (
    listingId: string,
    updatedFields: Partial<WasteListing>
  ) => {
    setWasteListings((prev) =>
      prev.map((listing) =>
        listing.id === listingId ? { ...listing, ...updatedFields } : listing
      )
    );
    await new Promise((resolve) => setTimeout(resolve, 200));
    console.log(`Listing ${listingId} updated:`, updatedFields);
  };
