...
function MapInteractions({ locations, focusListingId }: Omit<MapClientProps, 'onMarkerClick'>) {
  const map = useMap();
  useEffect(() => {
    setTimeout(() => {
      map.invalidateSize();
  ...
  useEffect(() => {
    if (focusListingId) {
      ...
      if (loc?.location.latitude != null && loc.location.longitude != null) {
        map.flyTo([loc.location.latitude, loc.location.longitude], 15, { animate: true, duration: 1 });
      }
    }
...
export default function MapClient({ locations, onMarkerClick, focusListingId }: MapClientProps) {
  const getIcon = useCallback((type: ItemType, cat?: WasteCategory) => {
    const baseColor = type === 'old_item'
      ? '#FF9800'
      : cat === 'hazardous'
      ? '#F44336'
      : cat === 'e_waste'
  ...
  return (
    <MapContainer key={focusListingId ?? 'default'} center={[27.1751,78.0421]} zoom={13} scrollWheelZoom className={styles.mapComponent}>
        ...
        l.location.latitude != null && l.location.longitude != null && (
          ...
          >
            <Popup>
              <h3>{l.wasteType}</h3>
              <p>{l.description}</p>
              <p><strong>Qty:</strong> {l.quantity} {l.unit}</p>
